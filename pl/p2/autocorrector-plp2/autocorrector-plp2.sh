...
PRINCIPAL=plp2      # Nombre del ejecutable de la práctica
...
function prueba() {
	...
	$mata $MAXTIME $OBT/$bn.sal java -classpath $ANTLRJAR:. $PRINCIPAL $ENT/$i > $OBT/$bn.sal 2> $OBT/$bn.err

	$comparefiles $OBT/$bn.sal $SAL/$bn.sal
	if [ $? -ne 0 ]; then
	    echo "--- Fallo en la traduccion ----------"
	    diff -EwB -I "---" $OBT/$bn.sal $SAL/$bn.sal 2>&1
	else 
...
echo -e "\n  Los programas de prueba son los utilizados en la práctica 1, excepto los"
echo -e " que generaban errores léxicos y sintácticos.\n"
