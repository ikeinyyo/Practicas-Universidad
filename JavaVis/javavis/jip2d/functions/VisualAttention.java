 ...
 * </ul><br />
 */
public class VisualAttention extends Function2D {
...
//		ret.setAllPixels(21, ((JIPBmpFloat)FS.processImg(mapaOr0.get(5))).getAllPixels());
	...
	public Sequence processSeq(Sequence seq) throws JIPException
	{
		...
		{
			res.addFrame(filterRed.get(count));
		}
		for (count=0; count<NUM_SCALES; count++)
		{
			res.addFrame(filterGreen.get(count));
		}
		for (count=0; count<NUM_SCALES; count++)
		{
			res.addFrame(filterBlue.get(count));
		}
		for (count=0; count<NUM_SCALES; count++)
		{
			res.addFrame(filterYellow.get(count));
		}
...
//		normalizeRange0R(oc, 1);
		...
		{
			vr[count] = (vi[count] + vc[count] + vo[count]) / 3;
		}
